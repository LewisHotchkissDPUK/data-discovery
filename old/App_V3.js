...
const App = () => {
  ...
  const [selectedAICohorts, setSelectedAICohorts] = useState([]);
  ...
  // Intersection observer for infinite scroll in tables
  const sentinelRefs = useRef({});
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          const tableKey = Object.keys(sentinelRefs.current).find(key => sentinelRefs.current[key] === entry.target);
          if (tableKey) {
            setExpandedTables(prev => {
              const current = prev[tableKey] || { expanded: true, shown: 20 };
              return { ...prev, [tableKey]: { ...current, shown: current.shown + 20 } };
            });
          }
        }
      });
    ...
    Object.values(sentinelRefs.current).forEach(el => el && observer.observe(el));
  ...
  const handleFileUpload = async (e) => {
    ...
    // Initialize selected AI cohorts
    const totalVars = newAllCohorts.reduce((sum, c) => sum + c.variableCount, 0);
    setSelectedAICohorts(totalVars <= 5000 ? newAllCohorts : []);
    
    // Initialize expanded tables state with all collapsed
    const allTables = [...new Set(newAllVars.map(v => `${v.cohort_name}-${v.table_name}`))];
    setExpandedTables(prev => {
      const newState = { ...prev };
      allTables.forEach(key => {
        if (!(key in newState)) newState[key] = { expanded: false, shown: 0 };
      });
      return newState;
    });
  ...
  const getAISuggestions = async () => {
    ...
    const filteredVariables = variables.filter(v => selectedAICohorts.some(c => c.name === v.cohort_name));
    if (filteredVariables.length === 0) return; // No cohorts selected or no variables
...
Available variables:
${filteredVariables.map(v => `- ${v.variable_name} (${v.cohort_name}): ${v.variable_description} [Table: ${v.table_name}]`).join('\n')}
...
[
  ...
  const handleCollapseAll = () => {
    const newExpanded = {};
    variables.forEach(v => {
      const key = `${v.cohort_name}-${v.table_name}`;
      newExpanded[key] = { expanded: false, shown: 0 };
    });
                ...
                <div className="flex justify-end gap-3 px-2">
                   <button 
                     onClick={() => {
                       ...
                       variables.forEach(v => {
                         const key = `${v.cohort_name}-${v.table_name}`;
                         newExpanded[key] = { expanded: true, shown: 20 };
                       });
                       setExpandedTables(newExpanded);
                     }}
                      ...
                      <div key={cohortName} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                        ...
                        <div className="divide-y divide-slate-100">
                          {tablesInCohort.map(tableName => {
                            ...
                            const tableState = expandedTables[tableKey] || { expanded: false, shown: 0 };
                            ...
                            return (
                              <div key={tableName}>
                                <div 
                                  ...
                                  onClick={() => {
                                    const current = expandedTables[tableKey] || { expanded: false, shown: 0 };
                                    const newExpanded = !current.expanded;
                                    setExpandedTables(prev => ({ ...prev, [tableKey]: { expanded: newExpanded, shown: newExpanded ? 20 : 0 } }));
                                  }}
                                >
                                  <div className="flex items-center gap-3">
                                ...
                                {isExpanded && (
                                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 p-4 bg-slate-50/50">
                                    {(() => {
                                      const shown = tableState.shown;
                                      const displayedVars = tableVars.slice(0, shown);
                                      return (
                                        <>
                                          {displayedVars.map(v => {
                                          ...
                                          {tableState.shown < tableVars.length && <div ref={el => sentinelRefs.current[tableKey] = el} className="h-4 col-span-full" />}
                                        </>
                                      );
                                    })()}
                                  </div>
                ...
                <div className="flex-1 bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col">
                  ...
                  <div className="px-4 py-3 bg-white border-b border-slate-200">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-slate-700">Query Cohorts:</span>
                      <div className="relative">
                        <button
                        ...
                        >
                          <span>{selectedAICohorts.length === 0 ? 'Select cohorts...' : `${selectedAICohorts.length} selected`}</span>
                          <ChevronDown className={`w-3 h-3 transition-transform ${isCohortDropdownOpen ? 'rotate-180' : ''}`} />
                        </button>
                        {isCohortDropdownOpen && (
                          <div className="absolute top-full left-0 mt-1 w-64 bg-white border border-slate-200 rounded-lg shadow-lg z-10 max-h-60 overflow-y-auto">
                            {cohorts.map((cohort) => {
                              const isSelected = selectedAICohorts.some(c => c.name === cohort.name);
                              const totalSelectedVars = selectedAICohorts.reduce((sum, c) => sum + c.variableCount, 0);
                              const wouldExceed = !isSelected && totalSelectedVars + cohort.variableCount > 5000;
                              
                              return (
                                <label key={cohort.name} className="flex items-center gap-2 px-3 py-2 hover:bg-slate-50 cursor-pointer">
                                  <input
                                    type="checkbox"
                                    checked={isSelected}
                                    onChange={() => {
                                      if (isSelected) {
                                        setSelectedAICohorts(prev => prev.filter(c => c.name !== cohort.name));
                                      } else if (!wouldExceed) {
                                        setSelectedAICohorts(prev => [...prev, cohort]);
                                      }
                                  ...
                                  <div className="min-w-0 flex-1">
                                    <div className="flex items-center justify-between">
                                      <span className="text-sm font-medium text-slate-700 truncate">{cohort.name}</span>
                                      <span className="text-xs text-slate-500">{cohort.variableCount} vars</span>
                                    </div>
                                    {wouldExceed && (
                                      <p className="text-xs text-red-500 mt-0.5">Exceeds 5000 variable limit</p>
                                    )}
                                  </div>
                                </label>
                              );
                            ...
                            <div className="px-3 py-2 border-t border-slate-200 bg-slate-50">
                              <p className="text-xs text-slate-600">
                                Total variables: {selectedAICohorts.reduce((sum, c) => sum + c.variableCount, 0)} / 5000
                              </p>
                            </div>
                          </div>
                        )}
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    {chatMessages.length === 0 && (
                      <div className="h-full flex flex-col items-center justify-center text-center opacity-30">
                        ...
                        <p className="text-sm max-w-xs mt-1">Describe your research topic (e.g., "cognitive decline markers in early onset dementia") and I'll identify relevant variables across selected cohorts.</p>
                      </div>
                    )}
                  ...
                  <div className="p-4 border-t border-slate-200 bg-slate-50">
                    <div className="flex gap-2 relative">
                      <input 
                        ...
                        className="flex-1 px-5 py-3 bg-white border border-slate-200 rounded-2xl text-sm shadow-sm focus:ring-2 focus:ring-purple-500 outline-none transition-all pr-12 disabled:bg-slate-100 disabled:text-slate-400"
                        value={searchQuery}
                        onChange={e => setSearchQuery(e.target.value)}
                        onKeyPress={e => e.key === 'Enter' && selectedAICohorts.length > 0 && getAISuggestions()}
                        disabled={selectedAICohorts.length === 0}
                      />
                      <button 
                        onClick={getAISuggestions}
                        disabled={selectedAICohorts.length === 0}
                        className="absolute right-2 top-2 p-2 bg-purple-600 text-white rounded-xl hover:bg-purple-700 disabled:bg-slate-400 disabled:cursor-not-allowed transition-all shadow-md active:scale-95"
                      >
